...
**Status:** planning/eval complete, first theme (Hinokami Night) installed, dashboard boot blocked by a desktop-app dependency (see §3 Known Blocker). No Lars theme built yet.
...
## 3. Known blocker (open)

The v0.21.3 dashboard SPA **requires `window.hermesDesktop`** (injected only by the Hermes desktop app) and refuses to boot without it: "Desktop IPC bridge is unavailable" — in external browsers AND in the desktop preview pane. Verified by decompiling the served bundle (`index-CubyQ1sd.js`: `if(!t) return qi('Desktop IPC bridge is unavailable')`) and reproducing in-desktop. The official docs still claim plain-browser use works — likely a regression or desktop-first change in this build.

**Workarounds to try on next machine (with its Hermes):**
1. Fully restart the Hermes desktop app, then open the dashboard *from inside the app* (bridge present there)
2. Check whether a newer/older dashboard build restores standalone browser mode
3. If neither: the desktop app becomes the mandatory dashboard host, or pivot theme work to surface 3/4
